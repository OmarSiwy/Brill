...
fn libinputDeviceListener(
    ...
    switch (event) {
        .tap_support => |ev| {
            ...
            const state: river.LibinputDeviceV1.TapState = switch (wm.getConfig().input.tap) {
                .disabled => .disabled,
                .enabled => .enabled,
            };
        ...
        .natural_scroll_support => |ev| {
            ...
            const state: river.LibinputDeviceV1.NaturalScrollState = switch (wm.getConfig().input.natural_scroll) {
                .disabled => .disabled,
                .enabled => .enabled,
            };
        ...
        .click_method_support => |ev| {
            const method: river.LibinputDeviceV1.ClickMethod = switch (wm.getConfig().input.click_method) {
                .none => .none,
        ...
        .tap_button_map_default => {
            const map: river.LibinputDeviceV1.TapButtonMap = switch (wm.getConfig().input.tap_button_map) {
                .lrm => .lrm,
                .lmr => .lmr,
            };
        ...
        .clickfinger_button_map_default => {
            const map: river.LibinputDeviceV1.ClickfingerButtonMap = switch (wm.getConfig().input.clickfinger_button_map) {
                .lrm => .lrm,
                .lmr => .lmr,
            };
